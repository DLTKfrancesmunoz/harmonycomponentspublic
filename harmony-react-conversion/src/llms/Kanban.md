...
`Kanban.tsx` imports `KanbanCostpoint.css`. Ensure design tokens (`--kanban-cp-*`) are available on `html.theme-cp`. With `html.theme-cp.dark`, Kanban chrome and field-card surfaces follow the CP semi-dark palette (see `tokens.css`).
