...
**Key principle:** Never edit `node_modules/@deltek/harmony-design-system/`. Changes there are wiped on `npm install` and break production. Use the four-tier customization system instead.
...
|--------|-----|---------|-------------|
| **npm from Git** (recommended) | `npm install github:deltek/ux-harmony-design-system` | `npm update @deltek/harmony-design-system` and `npm run harmony:check-updates` | Most app projects; package lives in `node_modules` |
...
**Recommended path:** Install as a package from Git. Your app lists `@deltek/harmony-design-system` as a dependency; Harmony lives in `node_modules`. You get updates via npm and can use the helper scripts (`harmony:copy`, `harmony:check-updates`, `harmony:diff`).
...
Replace `<repo-url>` with your repository URL (e.g. `https://github.com/deltek/ux-harmony-design-system.git`).
...
```bash
npm install github:deltek/ux-harmony-design-system
```

Or with a specific version:

```bash
npm install git+https://github.com/deltek/ux-harmony-design-system.git#v1.0.1
```
...
```bash
mkdir -p scripts
cp node_modules/@deltek/harmony-design-system/scripts/copy-harmony-component.cjs scripts/
cp node_modules/@deltek/harmony-design-system/scripts/check-harmony-updates.cjs scripts/
cp node_modules/@deltek/harmony-design-system/scripts/diff-harmony-component.cjs scripts/
```
...
```bash
cp node_modules/@deltek/harmony-design-system/public/RS_DelaDefault.svg public/
cp node_modules/@deltek/harmony-design-system/public/RS_Dela_Active.svg public/
cp node_modules/@deltek/harmony-design-system/public/DelaChat.svg public/
cp node_modules/@deltek/harmony-design-system/public/Stars.svg public/
cp node_modules/@deltek/harmony-design-system/public/Risk\ Shield.svg public/
cp node_modules/@deltek/harmony-design-system/public/Report.svg public/
cp node_modules/@deltek/harmony-design-system/public/Resource.svg public/
cp node_modules/@deltek/harmony-design-system/public/related.svg public/
cp node_modules/@deltek/harmony-design-system/public/template.svg public/
cp node_modules/@deltek/harmony-design-system/public/DStar_LM.svg public/
```
...
```astro
---
import '@deltek/harmony-design-system/styles/reset.css';
import '@deltek/harmony-design-system/styles/tokens.css';
import '@deltek/harmony-design-system/styles/global.css';
import '@deltek/harmony-design-system/styles/components.css';
...
```astro
---
import { Button, Card, Input, Alert, Badge } from '@deltek/harmony-design-system/ui';
---
...
```astro
---
import Button from '@deltek/harmony-design-system/ui/Button.astro';
import Card from '@deltek/harmony-design-system/ui/Card.astro';
---
```
...
```astro
---
import ShellLayout from '@deltek/harmony-design-system/layouts/ShellLayout.astro';
import DocsLayout from '@deltek/harmony-design-system/layouts/DocsLayout.astro';
---
```
...
```astro
---
import * as tokens from '@deltek/harmony-design-system/tokens';
import colors from '@deltek/harmony-design-system/tokens/colors.json';
import spacing from '@deltek/harmony-design-system/tokens/spacing.json';
---
```
...
**Token definitions (JSON in package):** `node_modules/@deltek/harmony-design-system/src/tokens/colors.json`, `spacing.json`, `typography.json`, `elevations.json` (or via `@deltek/harmony-design-system/tokens/...`).
...
Import from `@deltek/harmony-design-system/ui` and use with props. No extra files.
...
```astro
import Button from '../../../node_modules/@deltek/harmony-design-system/src/components/ui/Button.astro';
```
...
**More examples:** The package includes full wrapper and fork examples (e.g. ProjectCard, LoginButton, StatusCard, ShellHeader/ShellLayout with custom props) in `node_modules/@deltek/harmony-design-system/examples/` (wrappers and composed folders).
...
1. **Update the package:**
   ```bash
   npm update @deltek/harmony-design-system
   ```
   Or: `npm install @deltek/harmony-design-system@latest`
...
- **Option 1:** `npm install @deltek/harmony-design-system@<previous-version>` (e.g. `@1.0.0`).
...
3. Ensure your app's `package.json` dependency uses the HTTPS URL (e.g. `github:deltek/ux-harmony-design-system` or `git+https://...`). Many platforms use `GITHUB_TOKEN` automatically for `npm install` when installing from GitHub.
...
```astro
import { Button, Card, Input } from '@deltek/harmony-design-system/ui';
import ShellLayout from '@deltek/harmony-design-system/layouts/ShellLayout.astro';
import '@deltek/harmony-design-system/styles/global.css';
import * as tokens from '@deltek/harmony-design-system/tokens';
```
...
- **Docs:** `node_modules/@deltek/harmony-design-system/README.md` and `node_modules/@deltek/harmony-design-system/docs/customization/` (this guide).
- **Scripts:** `node_modules/@deltek/harmony-design-system/scripts/`.
...
This project uses Harmony with the four-tier customization system. See the [Consumer Guide](node_modules/@deltek/harmony-design-system/docs/customization/CONSUMER_GUIDE.md) in the package.
...
2. **Converter subagent:** Copy `reference/converter-subagent.md.example` to `.cursor/agents/harmony-converter.md`. Replace `[SPARSE_CHECKOUT_OR_COPY_COMMAND]` with your repo-specific command to get the preview file onto disk (e.g. sparse-checkout or copy from `node_modules/@deltek/harmony-design-system/src/pages/preview/`).
...
5. **Conversion playbooks (Agent Skills):** Copy the skill folders `convert-component`, `convert-shell`, and `convert-all` from this repo’s `.cursor/skills/` into your project’s `.cursor/skills/` (each folder contains a `SKILL.md` with `disable-model-invocation: true`). Invoke them from chat with `/convert-component`, `/convert-shell`, or `/convert-all`. The stubs under `reference/commands/` only point to those skills—do not copy them as the source of truth.
