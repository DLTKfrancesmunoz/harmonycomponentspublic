...
These rows are **natural-language** capabilities (no separate command files)—trigger them by asking in chat; this skill defines how to respond.

**Explicit playbooks** (separate Agent Skills under `.cursor/skills/<name>/`, `disable-model-invocation: true`, invoke via `/` e.g. `/build-layout`): `convert-component`, `convert-shell`, `convert-all`, `build-layout`, `build-all-patterns`, `create-pattern`, `search-patterns`, `seed-patterns`, `generate-llms-txt`, `harmony-critique`, `ux-review`. See [DESIGNER_GUIDE.md](../../DESIGNER_GUIDE.md) for a full table.
...
2. Else: **Harmony repo** = workspace root; **Consumer project** = `node_modules/@dltkfrancesmunoz/harmony-design-system` or the path where components were copied (e.g. `src/harmony`).
