...
0. **Check Harmony installation (before resolving paths).**
   ...
   - Else if `node_modules/@deltek/harmony-design-system/` exists: package is installed → proceed.
...
2. **Build the component list.**
   - Scan `node_modules/@deltek/harmony-design-system/src/components/ui/*.astro` (or the resolved Harmony root) for all `.astro` files. Each file is one component (filename without extension = component name).
