...
Quick reference for Cursor **Agent Skills** and design workflows in this repo. Playbooks that used to live as project slash commands now live under `.cursor/skills/<name>/SKILL.md` with `disable-model-invocation: true`—invoke them the same way in chat (type `/` and choose the skill name, e.g. `/build-layout`, `/convert-shell`).

---

## Slash commands

These workflows are **explicit-invocation skills**—pick them from the `/` menu (or type `/build-layout`, etc.) so the agent loads that playbook.
...
| **/convert-shell** | Convert the full Harmony shell (header, footer, sidebars, page header, content slot) to your target framework. You provide shell variant (e.g. "VP dark", "CP dark") and framework. Converts leaf components first (through Avatar), then assembles ShellLayout, then runs integration verification. |
...
| **/build-all-patterns** | Build one implemented page per pattern from the design-patterns registry (target framework required). Each page uses its own shell instance and implements that pattern in the content slot. Optional `--dry-run`, `--category [name]`, `--start-from [slug]`. Runs **layout-verifier** and **pattern-fidelity-verifier** (or equivalent checks) on each; checkpoints every 5 patterns for review. Requires converted shell and routing set up. |
...
### Pattern page fidelity (when using `/build-all-patterns`)

For files under `src/patterns/**/*`, the project rule **pattern-fidelity** requires the built page to match the pattern markdown: Component Tree, Key Elements, named labels, and counts must appear in the UI as specified (no stubs or simplified placeholders). After each pattern page is built, verification should include **layout-verifier** and **pattern-fidelity-verifier** (see `.cursor/agents/`) before moving on. Full detail: `.cursor/rules/pattern-fidelity-rule.md`.
...
6. **Build all pattern demo pages**  
   After the shell is converted and routing exists, run `/build-all-patterns [framework]` to implement one page per registry pattern. Use `--dry-run` to preview the list, `--category` to limit scope, or `--start-from` to resume. Expect layout + pattern-fidelity verification each time; see **Pattern page fidelity** above.

7. **Find existing patterns**  
   `/search-patterns wizard` or `/search-patterns --product PPM` or `/search-patterns --category navigation`.

8. **Review design for Harmony and UX**  
   `/harmony-critique` on the current file or design.

9. **Review UX only (any project)**  
