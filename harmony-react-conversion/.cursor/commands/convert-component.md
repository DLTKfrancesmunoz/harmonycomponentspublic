...
6. **Delegate to converter:** Invoke the **harmony-converter** agent with a prompt that @-mentions the resolved Source, Preview, and Output files and states the target framework. The converter subagent expects exactly those @-references.
...
**Important:** Read the component table from the harmony-converter skill (SKILL.md); do not ask the user for file paths — path resolution is your job from the table and project layout. If the preview file is missing, path resolution must point to an existing preview path or the user must run sparse-checkout/copy per the skill's CONSUMER_GUIDE; the converter subagent will output BLOCKED if the preview file is not found. If the verifier or converter reports missing/unresolved icons (e.g. "?" fallback), the converter must check the local project's Icon component and icon config (e.g. Icon.vue, iconPaths) and add missing path or inline data so every icon used is either custom img or inline SVG.
