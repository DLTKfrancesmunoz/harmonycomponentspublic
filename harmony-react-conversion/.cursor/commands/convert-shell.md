...
3. **Leaf components (in order):** For each of: Icon, LeftSidebar, RightSidebar, TabStrip, ShellFooter, ShellPageHeader, Card:
...
**Note:** The skill's order is: leaf components first (Icon through Card), then ShellLayout (assembly), then integration verify. The integration step uses the two reference inputs above so the verifier knows what "integration diff" means.
