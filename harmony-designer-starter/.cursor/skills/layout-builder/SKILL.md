...
- **Harmony React components** → implementation lives under `src/components/harmony/` (one folder per component, `.tsx` + `.css`). Use these files as the source of props and structure—not Astro sources.
...
- Document-level selectors need global CSS when applicable.
