...
Use these paths when answering questions in **this** repository (self-contained preview, not the Astro Harmony package):
...
- **Global Harmony CSS:** `harmony-styles/` (loaded through `src/main.tsx` via the `@deltek/harmony-components/styles/*` alias).
- **Icon manifest:** `harmony-data/icon-manifest.json` (alias `@harmony-data`).
- **Slash commands** available here are listed in [`.cursor/DESIGNER_GUIDE.md`](../DESIGNER_GUIDE.md) (e.g. `/build-layout`, `/harmony-critique`, `/ux-review`). Generic `/harmony-*` names below are conceptual unless a matching command file exists.

The Astro source layout (`src/components/ui/*.astro`) applies to the upstream design system repo, not this starter—prefer the React paths above.
